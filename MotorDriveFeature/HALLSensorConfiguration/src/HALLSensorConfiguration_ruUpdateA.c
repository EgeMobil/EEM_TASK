...
{
    dtHALLSensorConfiguration_PwmStep pwmStepA = PWMSTEP_PWMSTATUS_UNKNOWN;

    /* Get Singleton Instance */
    dtHALLSensorConfiguration* hsc = HALLSensorConfiguration_GetInstance();

    /* Read HALL from interfaces */
    hsc->getHallAConfig().setRisingEdgeFlag( hsc->IHALLA->readRisingEdgeAFlag() );
    hsc->getHallAConfig().setFallingEdgeFlag( hsc->IHALLA->readFallingEdgeAFlag() );

    /* Set pwmStep for HALL A*/
    if(( HALLA_RISING_EDGE_SET == hsc->getHallAConfig().getRisingEdgeFlag() ) && ( HALLA_FALLING_EDGE_RESET == hsc->getHallAConfig().getFallingEdgeFlag() ) )
    {
        pwmStepA = PWMSTEP_PWMSTATUS_HIGH;
        /* Local handlers for debug */
        hsc->getHallAConfig().setStatus(HALLA_STATUS_HIGH);
    }
    else if(( HALLA_RISING_EDGE_RESET == hsc->getHallAConfig().getRisingEdgeFlag() ) && ( HALLA_FALLING_EDGE_SET == hsc->getHallAConfig().getFallingEdgeFlag() ) )
    {
        pwmStepA = PWMSTEP_PWMSTATUS_LOW;
        /* Local handlers for debug */
        hsc->getHallAConfig().setStatus(HALLA_STATUS_LOW);
    }
    else
    {
        /* Maintain previous status */
        pwmStepA = ( hsc->getHallAConfig().getStatus() == HALLA_STATUS_HIGH) ? PWMSTEP_PWMSTATUS_HIGH : PWMSTEP_PWMSTATUS_LOW;
